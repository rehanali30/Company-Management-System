...
if (strlen($_SESSION['uid']==0)) {
  ...
  if(isset($_POST['submit']))
  {
    ...
    $query=mysqli_query($con,"SELECT ID FROM emp_detail WHERE ID='$empid' AND 	EmpPassword='$cpassword'");
    $row=mysqli_fetch_array($query);
    if($row>0) {
      $ret=mysqli_query($con,"UPDATE emp_detail SET EmpPassword='$newpassword' WHERE ID='$empid'");
      $msg= "Your Password is Successully Changed"; 
    }
    else {
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <form name="changepassword" class="user" method="post" onsubmit="return checkpass();">
            <?php
              $cid=$_SESSION['uid'];
              $ret=mysqli_query($con,"SELECT * FROM emp_detail WHERE ID='$cid'");
