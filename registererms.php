...
if(isset($_POST['submit']))
{
  ...
  $ret=mysqli_query($con, "SELECT EmpEmail FROM emp_detail WHERE EmpEmail='$Email'");
  $result=mysqli_fetch_array($ret);
  if($result>0) {
    $msg="This Email Already Associated with Another Account";
  }
  else {
    $query=mysqli_query($con, "INSERT INTO emp_detail(EmpFname, EmpLName, EmpCode, EmpEmail,  EmpPassword) VALUE('$FName', '$LName', '$empcode', '$Email', '$Password' )");
    if ($query) {
...
<head>
  ...
  <title>Company Managment System</title>
